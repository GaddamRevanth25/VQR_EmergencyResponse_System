...
class NotificationService:
    ...
    def send_crash_sos_notifications(
        ...
        """Send formatted emergency SMS with Google Maps link and HTML Email to emergency contacts."""
        maps_link = (
        ...
        c_name = contact_name.strip() if (contact_name and contact_name.strip()) else "Emergency Contact"
        u_name = user_name.strip() if (user_name and user_name.strip()) else "VQR User"
        u_phone = user_phone.strip() if (user_phone and user_phone.strip()) else "N/A"
        veh_no = vehicle_number.strip() if (vehicle_number and vehicle_number.strip()) else "N/A"
        rel_str = f" ({contact_relation.strip()})" if (contact_relation and contact_relation.strip()) else ""
        lat_str = str(latitude) if latitude is not None else "N/A"
        lon_str = str(longitude) if longitude is not None else "N/A"

        sms_body = (
            f"🚨 EMERGENCY ALERT 🚨\n\n"
            f"Dear {c_name},\n\n"
            f"This is an automated SOS alert from VQR Emergency Response System.\n\n"
            f"{u_name}{rel_str} has triggered an emergency alert.\n\n"
            f"👤 User Details:\n"
            f"Name: {u_name}\n"
            f"Phone: {u_phone}\n"
            f"Vehicle Number: {veh_no}\n\n"
            f"🚨 Emergency Status:\n"
            f"An emergency situation has been detected. Immediate assistance may be required.\n\n"
            f"📍 Current Location:\n"
            f"{maps_link}\n\n"
            f"Coordinates:\n"
            f"Latitude: {lat_str}\n"
            ...
            f"⏰ Time:\n"
            f"{created_at_str}\n\n"
            f"Please try contacting {u_name} immediately or reach the location if assistance is needed.\n\n"
            f"This alert was automatically generated by the VQR Emergency Response System."
        )

        twilio_configured = bool(
            settings.TWILIO_ACCOUNT_SID and settings.TWILIO_AUTH_TOKEN and (settings.TWILIO_PHONE_NUMBER or settings.TWILIO_VERIFY_SERVICE_SID)
        )

        if not twilio_configured:
            _logger.warning("Twilio credentials not fully configured in environment variables. Simulating emergency dispatch.")
            _logger.info(
                f"\n=== MOCK TWILIO NOTIFICATION DISPATCHED ===\n"
                f"To: {contact_phone or user_phone or 'N/A'}\n"
                f"Message:\n{sms_body}\n"
                f"===========================================\n"
            )

        # ── SMS ──
        target_phone = contact_phone or user_phone
        if target_phone:
            try:
                NotificationService._send_sms(target_phone, sms_body)
                _logger.info(f"🚨 Emergency SOS SMS dispatched to {target_phone}")
            except Exception as e:
                _logger.error(f"Failed to send emergency SMS to {target_phone}: {e}")
            ...
            try:
                NotificationService._send_voice_call(target_phone, spoken_alert)
            except Exception as e:
                _logger.error(f"Failed to trigger emergency voice call to {target_phone}: {e}")
